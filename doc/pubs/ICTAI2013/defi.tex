...
%that we must maximize the profit given a reduction plan of $M$ years, given:
%
%\begin{itemize}
%    \item a yearly budget for a set of resources $o_{il}$, $1 \le i \le M$, $1 \le l \le L$;
%    
%    \item a fraud reduction goal $G_i$, $1 \le i \le M$ that represents the
%    amount of energy loss that must be reduced;
%
%    \item the energy value $v$, that represents the value of the unit of energy.
%\end{itemize}
%
%We also assume that there are several action to choose from a portfolio of actions of size $N$.
%Each action $j$, $1 \le j \le N$, contained in the portfolio has several parameters: 
%\begin{itemize}
%    \item $m_j$, the maximum number of times that the action $j$ may be performed, we
%    shall refer to it as \textit{the market} of the action;
%    \item $c_{j,l}$, the $l$-th cost associated to each execution of action $j$;
%    \item $e_{j,k}$, the amount of energy that the action $j$ will recover after 
...
%    The action $D_{j,d}$ ($1 \le d \le D_j$) must be executed a number of times defined by
%    \item the parameter $Q_{j,d} \subset \mathbb{N}$.
%\end{itemize}
%
...
%of values that maximizes the profit and is under the fraud reduction for all years. 
%
%The constraints of the problem are the yearly budget restriction,
%\begin{equation}
%    \sum_{j=1}^{N} x_{j, i} \times c_{j,l} \le o_{i,l}, \forall i, l,
%\end{equation}
...
%\begin{equation}
%    \label{eq:goal}
%    R_i(\bar{x}) \leq G_i, \forall i, \\
%\end{equation}
%$R_i$ being the fraud reduction in the year $i$, defined as
%\begin{equation}
%    \label{eq:rec}
%    R_k(\bar{x}) = \sum_{i=1}^{k} \sum_{j=1}^{N} x_{j, i} \times e_{j, k - i + 1},
%\end{equation}
%and the dependency restriction for all actions and for all years,
%\begin{align}
%    \label{eq:dependency}
%    \forall j,k \sum_{i=1}^{k} x_{d, i} \ge Q_{j, d}, \forall d \in D_j
%\end{align}
%
%To introduce the objective function we must define some auxiliary concepts:
%The total solution cost, $C$,
%\begin{equation}
%\label{eq:cost}
%C(\bar{x}) =  \sum_{i=1}^{M} \sum_{j=1}^{N} \sum_{l=1}^{L} x_{j, i} \times c_{j,l},
%\end{equation}
%the organically gained profit, due to elimination of the fraud, $V$,
%\begin{equation}
%    V(\bar{x}) = v \times \sum_{i=1}^{M} R_i(\bar{x}),
%\end{equation} 
%finally, we may define the objective function as:
%\begin{align}
%    \label{eq:objective}
%    max(O(\bar{x})) = &  max(\sum_{l=1}^{L} \sum_{i=1}^{M} o_{i,l} - C(\bar{x}) + V(\bar{x}))\\
%    & = max(-C(\bar{x}) + V(\bar{x}))
%\end{align}                               
%The objective function represents the profit, that is,
%the remaining money $\sum_{i=1}^{M} o_i - C$ 
%plus the regained money through the energy $V$, comming from fraud reduction.
...
%
%, all the budget will be used and 
%$-C_i(x) \approx \sum_{i=1}^{M}\sum_{}o_{i,l}$
%it suffices to maximize the sum of the reduction energy defined in equation~\ref{eq:rec}. 
%Hence, we may re-write the objective function as:
%\begin{equation}
%    \label{eq:objective2}
%    max(O(\bar{x})) = max\left(\sum_{i=1}^{M} \frac{\sum_{j=1}^{N}R_{i,j}(\bar{x})}{(1+r)^i}\right).
%\end{equation}
