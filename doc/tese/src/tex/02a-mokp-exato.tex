...
uma vez que, ao ser o valor de $W - \sum_{i=k}^n$ computado, a relação $\rel_k^r$
...
\begin{displaymath}
  \forall s_k, s_{\til{k}} \in S_k, \; s_k \rel_k^{\dom} s_{\til{k}}
    \Leftrightarrow
    \begin{cases}
...
$s_k$.
A seguinte proposição mostra que $\rel_k^{\dom}$ é de fato uma relação de
dominância e apresenta propriedades adicionais de $\rel_k^{\dom}$.

\begin{myprop}[Relação $D_k^r$]
  \noindent
  \begin{enumerate}
    \item[(a)] $D_k^{\dom}$ é uma relação de dominância
    \item[(b)] $D_k^{\dom}$ é transitiva
    \item[(c)] $D_k^{\dom}$ admite ordem de preservação de dominância
    \item[(c)] $D_n^{\dom} = \dom$
\end{enumerate}
\end{myprop}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item[(a)]{ TODO.... }
    \item[(b)]{Trivial.}
    \item[(c)]{Pela Proposição~\ref{prop:domorder}, uma vez que $\rel_k^{\dom}$ é transitiva.}
...
\begin{equation}
  ...
  \label{eq:upperb}
\end{equation}
Finalmente definimos $\rel_k^b$ uma relação de dominância como caso particular
do tipo geral para $k = 1, \ldots, n$ por:
\begin{displaymath}
  \forall s_k, s_{\til{k}} \in S_k, \; s_k \rel_k^b s_{\til{k}}
    \Leftrightarrow
    \begin{cases}
...
A seguinte proposição mostra que $\rel_k^b$ é de fato uma relação de dominância
e apresenta propriedades adicionais de $\rel_k^b$.
...
\begin{proof}
  \noindent
  \begin{enumerate}
    \item[(a)]{ Considere os estados $s_k$ e $s_{\til{k}}$ tal que $s_k \rel_k^b s_{\til{k}}$.
      ...
      o que estabelece que $\rel_k^b$ satisfaz a condição da Equação~\ref{eq:domrel}
      da Definição~\ref{def:domrel}.
      }
    \item[(b)]{ Considere os estados $s_k, s_{\til{k}}$ e $s_{-k}$ tal que
      $s_k \rel_k^b s_{\til{k}}$ e $s_{\til{k}} \rel_k^b s_{-k}$.
      ...
      Portanto temos que $s_k \rel_k^b s_{-k}$. }
    \item[(c)]{Pela Proposição~\ref{prop:domorder}, uma vez que $\rel_k^b$ é transitiva.}
    \item[(d)]{Por definição.} \qedhere
  \end{enumerate}
\end{proof}
A relação $\rel_k^b$ é mais difícil de ser verificada do que as relações
$\rel_k^r$ e $\rel_k^{\dom}$ uma vez que requer uma maior quantidade de
comparações e informações sobre outros estados.

Obviamente, a relação $\rel_k^b$ seria mais forte se utizados complementos
...
Porém, no contexto em questão, uma vez que temos que verificar $\rel_k^b$
para muitos estados, enriquecer $\rel_k^b$ dessa forma demandaria um
esforço computacional alto demais.

\subsection{Utilizando multiplas relações de dominância}
